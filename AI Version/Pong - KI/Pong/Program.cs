...
namespace Pong
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Thread AiThread = new Thread(() => playAi(ball, leftPaddle));
            arrowKeysThread.Start();
            BallThread.Start();
            AiThread.Start();
        ...
        public static void playAi(Ball ball, Paddle leftPaddle)
        {
            Program p = new Program();
            while (true)
            {
                System.Threading.Thread.Sleep(75);
                if (ball.pos_y < leftPaddle.pos_y - 2 && ball.pos_x < FIELD_X / 4 && ball.velocity_x < 0)
                {
                    p.movePaddle(leftPaddle, true);
                }
                if (ball.pos_y > leftPaddle.pos_y + 2 && ball.pos_x < FIELD_X / 4 && ball.velocity_x < 0)
                {
                    p.movePaddle(leftPaddle, false);
                }
            }
        }
    }
